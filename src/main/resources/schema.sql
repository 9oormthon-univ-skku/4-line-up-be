...
create table area (id bigint generated by default as identity, name varchar(255) not null, summary varchar(255), description text not null, x integer, y integer, open timestamp(6), close timestamp(6), created_at timestamp(6) not null, modified_at timestamp(6) not null, primary key (id))
...
alter table if exists booth add constraint FK81p86gtlidlj9b4wyp66x1dgc foreign key (area_id) references area
alter table if exists booth add constraint FK2rrpioqdbuew45fwlxu8yhyev foreign key (category_id) references category
alter table if exists booth_image add constraint FK2owvids78jsimets3o73xqo9p foreign key (booth_id) references booth
alter table if exists booth_link add constraint FKofy8vjjy9jaubd42vikovhlww foreign key (booth_id) references booth
alter table if exists menu add constraint FK79sgcs9it1tx77sxxg1y4p808 foreign key (store_id) references booth
...
alter table if exists stop_time add constraint FKglo6p7gvk63smq8rsej4atkwu foreign key (stop_id) references booth
